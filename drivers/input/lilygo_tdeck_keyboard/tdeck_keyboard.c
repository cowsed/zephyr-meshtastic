...
static int set_brightness(const struct tdeck_kbd_config *cfg, uint8_t brightness) {
    ...
    int rc = i2c_write_read_dt(&cfg->bus, &tx_data, sizeof(tx_data), rx_buf, sizeof(rx_buf));
    if (rc < 0) {
...
static int set_restart_brightness(const struct tdeck_kbd_config *cfg, uint8_t brightness) {
    uint8_t tx_data = LILYGO_KB_ALT_B_BRIGHTNESS_CMD;
    uint8_t rx_buf[2] = {0};
...
static void keyboard_polling_thread(void *vp_cfg, void *vp_data, void *) {
    ...
    while (true) {
        ...
        if (last_restart_brightness != restart_brightness) {
            ...
            if (res < 0) {
                LOG_WRN("Failed to restart brightness: %d", res);
            }
